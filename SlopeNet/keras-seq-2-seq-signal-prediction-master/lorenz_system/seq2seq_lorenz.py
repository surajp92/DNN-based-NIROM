...
xtrain = np.zeros((nsamples, input_sequence_length, num_input_features))
ytrain = np.zeros((nsamples, input_sequence_length, num_output_features))
...
for k in range(nsamples):
    ...
    xt, yt = random_lorenz(states, input_sequence_length)
    
    xtrain[k,:,:] = xt
    ytrain[k,:,:] = yt

decoder_input = np.zeros((nsamples, 1, 1))
...
input_sequence_length = int(n/2) # Length of the sequence used by the encoder
target_sequence_length = int(n/2) # Length of the sequence predicted by the decoder
...
#train_data_generator = random_sine(batch_size=batch_size,#*steps_per_epoch,
...
#
#x_e = train_data_generator[0][0]  
#x_d = train_data_generator[0][1]  
#y_d = train_data_generator[1]
...
#%%
#test_data_generator = random_sine(batch_size=1000,#*steps_per_epoch,
#                                  steps_per_epoch=steps_per_epoch,
#                                  input_sequence_length=input_sequence_length,
#                                  target_sequence_length=target_sequence_length,
#                                  min_frequency=0.1, max_frequency=10,
...
#xt_e = test_data_generator[0][0]
#xt_d = test_data_generator[0][1]
#yt_d = test_data_generator[1]
#%%
...
#y_test_predicted = model.predict([xt_e, xt_d])
#%%
...
dt = 0.01
t = np.arange(t_init, t_final, dt)
num_steps_to_predict = int((t_final-t_init)/dt-input_sequence_length)

state0_test = [-8.0, 7.0, 27.0]
states_test = odeint(f, state0_test, t)

xtest = np.zeros((1,input_sequence_length, num_input_features))

xt, yt = random_lorenz(states_test, input_sequence_length)

xtest[0] = xt

#%%
x_encoder_test = xtest
x_decoder_test = np.zeros((1, input_sequence_length ,1))
...
for i in range(ny):
    ...
    axs[i].plot(t[:input_sequence_length],xtest[0,:,i], color='red', linestyle='--', label=r'$y_'+str(i+1)+'$'+' (True)', zorder=5)
    axs[i].plot(t[input_sequence_length:],y_test_predicted[0,:,i], color='blue', linestyle='-.', label=r'$y_'+str(i+1)+'$'+' (GP)', zorder=5) 
