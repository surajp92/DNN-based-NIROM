...
#%%
f_lorenz = np.zeros((nsamples,3))
f_lorenz[:,0] = sigma*(states[:,1] - states[:,0])
f_lorenz[:,1] = states[:,0]*(rho-states[:,2]) - states[:,1]
f_lorenz[:,2] = states[:,0]*states[:,1] - beta*states[:,2]
...
def create_training_data(training_set1, dt, t, nsamples):
    ...
    ytrain = [2.0*(training_set1[i+1,:3]-training_set1[i,:3])/dt-training_set1[i,3:] for i in range(m-1)]
    ytrain = np.array(ytrain)
    t = t.reshape(nsamples,1)
    xtrain = training_set1[0:m-1,:3]
    return xtrain, ytrain

#%%
training_set1 = np.hstack((states, f_lorenz))
...
#%%
#testing_set = np.hstack((states, f_lorenz))
testing_set = states
...
for i in range(1,my):
    ...
    yt_ml[i] = yt_ml[i-1] + 0.5*dt*(yt_sc + f_old)# y1 at next time step
    ytest[i,:3] = yt_ml[i]
    f_old = yt_sc
    f_old = f_old.reshape(1,ny)
    #ytest[i,3:] = yt_sc
