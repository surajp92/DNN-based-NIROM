...
slopenet = "RESNET" # Choices: BDF2, BDF3, BDF4, SEQ, EULER, LEAPFROG, LEAPFROG-FILTER
sigma = 0.00
problem = "ROM"
...
history_callback = custom_model.fit(xtrain, ytrain, epochs=900, batch_size=40, verbose=1, validation_split= 0.1,
...
indices_test = [int(dt_scaling*i) for i in range(int(m/dt_scaling)+1)]

#--------------------------------------------------------------------------------------------------------------#
# predict results recursively using the model 
start_test = cputime.time()
dt1 = dt
m = int(m/dt_scaling)+1
ytest_ml = model_predict(testing_set, m, n, dt1, legs, slopenet, sigma, sc_input, sc_output)
m = m-1 # for 2001 in next iterations
...
time = time-900
time_test = time.reshape(int(dt_scaling*m)+1,1)
testing_set = np.hstack((time_test, testing_set))
time_mlpred = time[indices_test]
time_mlpred = time_mlpred.reshape(m+1,1)
