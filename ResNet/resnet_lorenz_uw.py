...
state0 = np.zeros((rsamples, 3))

#%%
for k in range(rsamples):
    state0[k,:] = 30*(np.random.rand(3)-0.5)
    states = odeint(f, state0[k,:], t)
    m,n = states.shape

    xt, yt= create_training_data(states, dt, legs, slopenet)
    if k == 0:
        xtrain = xt
        ytrain = yt
    else:
...
indices = np.random.randint(0,xtrain.shape[0],10000)
...
#state0t = [-8.0, 7.0, 27.0]
state0t = 30*(np.random.rand(3)-0.5)
statest = odeint(f, state0t, t)
testing_set = statest
